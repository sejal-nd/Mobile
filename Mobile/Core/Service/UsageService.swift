...
protocol UsageService {
    ...
    ///   - billDate: the bill reference date
...
extension UsageService {
    ...
    func fetchBillComparison(accountNumber: String, premiseNumber: String, billDate: Date, yearAgo: Bool, gas: Bool) -> Observable<BillComparison> {
        return Observable.create { observer in
            self.fetchBillComparison(accountNumber: accountNumber, premiseNumber: premiseNumber, billDate: billDate, yearAgo: yearAgo, gas: gas, completion: { (result: ServiceResult<BillComparison>) in
                switch (result) {
