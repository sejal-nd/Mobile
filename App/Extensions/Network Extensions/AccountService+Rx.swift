...
extension Reactive where Base == AccountService {
    ...
    static func fetchAccountDetails(accountNumber: String = AccountsStore.shared.currentAccount.accountNumber,
                                    ...
                                    includeMDM: Bool = true) -> Observable<AccountDetail> {
        return Observable.create { observer -> Disposable in
            AccountService.fetchAccountDetails(accountNumber: accountNumber, payments: payments, programs: programs, budgetBilling: budgetBilling, alertPreferenceEligibilities: alertPreferenceEligibilities, includeMDM: includeMDM) { observer.handle(result: $0) }
