...
class TapToPayViewModel {
    ...
    var canEditPaymentDate: Bool {
        ...
        if Configuration.shared.opco == .bge || Configuration.shared.opco.isPHI { // BGE can always future date adding PHI for future date
            return true
            ...
            if let walletItem = walletItem, walletItem.bankOrCard == .bank {
                ...
                if Configuration.shared.opco == .bge || Configuration.shared.opco.isPHI {
                    if paymentAmount < minPayment {
                        if (FeatureFlagUtility.shared.bool(forKey: .isLowPaymentAllowed)) && (paymentAmount < 5.0) && (paymentAmount > 0) {
                            return nil
                        } else {
