...
class AccountPicker: UIControl {
    ...
    func refresh(force: Bool = false) {
        if force || shouldRefresh {
            ...
            let finaledString = NSLocalizedString(Configuration.shared.opco == .bge ? "Stopped" : "Finaled", comment: "")
            ...
            "\(account.isFinaled ? (Configuration.shared.opco.isPHI ? "(\(inactiveString))" : "(\(finaledString))") : account.isLinked ? "(\(linkedString))":"")"
    ...
    @objc private func onAccountPickerPress() {
        FirebaseUtility.logEventV2(.accountPicker(parameters: [.press]))
