...
public enum Router {
    ...
    case recoverMaskedUsernameAuid(request: RecoverMaskedUsernameRequestWithAuid)
    ...
    public var path: String {
        ...
        case .recoverUsername, .recoverMaskedUsername, .recoverMaskedUsernameAuid:
    ...
    public var httpBody: HTTPBody? {
        switch self {
        case .passwordChangeAnon(let request as Encodable), .passwordChange(let request as Encodable), .accountLookup(let request as Encodable), .recoverPassword(let request as Encodable), .budgetBillingUnenroll(_, let request as Encodable), .autoPayEnroll(_, let request as Encodable), .updateAutoPay(_, let request as Encodable), .autoPayEnrollBGE(_, let request as Encodable), .updateAutoPayBGE(accountNumber: _, let request as Encodable), .outageStatusAnon(let request as Encodable, _), .scheduledPayment(_, let request as Encodable), .billingHistory(_, let request as Encodable), .compareBill(_, _, let request as Encodable), .autoPayUnenroll(_, let request as Encodable), .scheduledPaymentUpdate(_, _, let request as Encodable), .homeProfileUpdate(_, _, let request as Encodable), .alertPreferencesUpdate(_, let request as Encodable), .updateGameUser(_, let request as Encodable), .setAccountNickname(let request as Encodable), .reportOutage(_, let request as Encodable), .reportOutageAnon(let request as Encodable), .recoverMaskedUsername(let request as Encodable), .recoverMaskedUsernameAuid(let request as Encodable), .sendCodeLookup(let request as Encodable),.validateCodeAnon(request: let request as Encodable), .recoverUsername(let request as Encodable), .validateRegistration(let request as Encodable), .checkDuplicateRegistration(let request as Encodable), .registration(let request as Encodable), .sendConfirmationEmail(let request as Encodable), .validateConfirmationEmail(let request as Encodable), .paperlessEnroll(_, let request as Encodable), .fetchDailyUsage(_, _, let request as Encodable), .registerForAlerts(let request as Encodable), .setAlertLanguage(_, let request as Encodable), .walletEncryptionKey(let request as Encodable), .wallet(let request as Encodable), .addWalletItem(let request as Encodable), .updateWalletItem(let request as Encodable), .deleteWalletItem(let request as Encodable), .scheduleOverride(_, _, _, let request as Encodable), .updateDeviceSettings(_, _, _, let request as Encodable), .updateThermostatSchedule(_, _, _, let request as Encodable), .fetchToken(let request as Encodable), .refreshToken(let request as Encodable), .meterPingAnon(let request as Encodable, _), .updateReleaseOfInfo(_, let request as Encodable), .scheduledPaymentDelete(_, _, let request as Encodable), .fetchDPA(_ , let request as Encodable), .fetchB2CJWT(let request as Encodable), .workDays(let request as Encodable), .stopServiceVerification(let request as Encodable), .stopISUMService(let request as Encodable),.streetAddress(let request as Encodable),.appartment(let request as Encodable), .addressLookup(let request as Encodable), .moveISUMService(let request as Encodable), .accountDetailsAnon(let request as Encodable), .workDaysAnon(let request as Encodable), .stopServiceVerificationAnon(let request as Encodable), .streetAddressAnon(let request as Encodable),.appartmentAnon(let request as Encodable), .addressLookupAnon(let request as Encodable), .moveISUMServiceAnon(let request as Encodable), .outageTracker(_, request: let request as Encodable), .thirdPartyTransferEligibility(let request as Encodable):
            return request.data()
    ...
    public var mockFileName: String {
        ...
        case .recoverUsername:
            fallthrough
        case .recoverMaskedUsername, .recoverMaskedUsernameAuid:
